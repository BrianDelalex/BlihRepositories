...
void check_and_print3(va_list *list, char const *form, int *i, int *ct)
{
    ...
    if (form[*i] == '%' && form[*i + 1] == 'x') {
        c_tmp = my_strcpy(convert_to_hexa(va_arg(*list, int), 32));
        *ct = *ct + my_strlen(c_tmp);
        my_putstr(c_tmp);
    }
    if (form[*i] == '%' && form[*i + 1] == 'X') {
        c_tmp = my_strcpy(convert_to_hexa(va_arg(*list, int), 0));
        *ct = *ct + my_strlen(c_tmp);
        my_putstr(c_tmp);
    }
    if (form[*i] == '%' && form[*i + 1] == '%') {
...
void check_and_print2(va_list *list, char const *form, int *i, int *ct)
{
    ...
    if (form[*i] == '%' && form[*i + 1] == 'b') {
        char_tmp = my_strcpy(convert_to_binarie(va_arg(*list, int)));
    ...
    if (form[*i] == '%' && form[*i + 1] == 'o') {
        char_tmp = my_strcpy(convert_to_octal(va_arg(*list, int)));
        *ct = *ct + my_strlen(char_tmp);
        my_putstr(char_tmp);
    }
...
void check_and_print(va_list *list, char const *form, int *i, int *ct)
{
    ...
    if (form[*i] == '%' && form[*i + 1] == 's') {
        char_tmp = my_strcpy(va_arg(*list, char *));
        *ct = *ct + my_strlen(char_tmp);
        my_putstr(char_tmp);
    }
